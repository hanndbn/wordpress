...
class ElatedSkin extends AmbientElatedClassSkinAbstract {
    /**
     * Skin constructor. Hooks to ambient_elated_admin_scripts_init and ambient_elated_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('ambient_elated_admin_scripts_init', array($this, 'registerStyles'));
        add_action('ambient_elated_admin_scripts_init', array($this, 'registerScripts'));

        add_action('ambient_elated_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('ambient_elated_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('ambient_elated_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('ambient_elated_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            ambient_elated_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            ambient_elated_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = ambient_elated_get_admin_tab();
        $active_page = $ambient_elated_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param AmbientElatedAdminPage $page current page to load
     *
     * @see ElatedSkinAbstract::loadTemplatePart()
     */
