(function($){
	...
	function eltdInitGalleryUploader() {
		...
		var $eltd_upload_button = jQuery('.eltdf-gallery-upload-btn');

		var $eltd_clear_button = jQuery('.eltdf-gallery-clear-btn');
			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=ambient_elated_gallery_upload_get_images&ids=" + ids,
	...
	function eltdInitPortfolioImagesVideosBox() {
		var eltd_portfolio_images = $('.eltdf-hidden-portfolio-images').clone().html();
		$portfolio_image = '<div class="eltdf-portfolio-images eltdf-portfolio-media" rel="">'+ eltd_portfolio_images +'</div>';
		var eltd_portfolio_videos = $('.eltdf-hidden-portfolio-videos').clone().html();

		$portfolio_videos = '<div class="eltdf-portfolio-videos eltdf-portfolio-media" rel="">'+ eltd_portfolio_videos +'</div>';
    ...
    */
    ...
    function eltdInitSlideElementItemAcc() {
        ...
        $(document).on('change', '#eltd_eltd_slide_holder_elements_alignment select', function(event) {
            var meta_box = $(this).parents('#eltdf-meta-box-eltd_slides_elements');
        ...
        $(document).on('change keyup', "input[name='eltd_slide_elements_default_width']", function(event) {
            $(this).parents('#eltdf-meta-box-eltd_slides_elements').find('.eltdf-slide-dynamic-def-width').html($(this).attr('value'));
        });
    ...
    function eltdInitSlideElementItemsBox() {
        //var eltd_slide_element = $('.eltdf-slide-element-additional-item-holder').clone().html();
        //$slide_element = '<div class="eltdf-slide-element-additional-item" rel="">'+ eltd_slide_element +'</div>';
    ...
    */
	...
	function eltdInitAjaxForm() {
		...
		$(document).delegate(".eltdf_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "ambient_elated_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
