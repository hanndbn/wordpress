<?php
class AmbientElatedClassLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_ambient_elated_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_ambient_elated_like', array( $this, 'ajax'));
	}
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'ambient');
		if( isset($_COOKIE['eltdf-like_'. $post->ID]) ){
			$class = 'eltdf-like liked';
			$title = esc_html__('You already like this!', 'ambient');
		}
...
if ( !function_exists( 'ambient_elated_create_like' ) ) {

    function ambient_elated_create_like() {
        AmbientElatedClassLike::get_instance();
    }

    add_action('after_setup_theme', 'ambient_elated_create_like');
}