...
if(!function_exists('ambient_elated_get_portfolio_category_list')) {
    function ambient_elated_get_portfolio_category_list($category = '') {

	    $number_of_items = 12;
	    $number_of_items_option = ambient_elated_options()->getOptionValue('portfolio_archive_number_of_items');
	    if(!empty($number_of_items_option)) {
		    $number_of_items = $number_of_items_option;
	    }

	    $number_of_columns = 4;
	    $number_of_columns_option = ambient_elated_options()->getOptionValue('portfolio_archive_number_of_columns');
	    if(!empty($number_of_columns_option)) {
		    $number_of_columns = $number_of_columns_option;
	    }

	    $space_between_items = 'normal';
	    $space_between_items_option = ambient_elated_options()->getOptionValue('portfolio_archive_space_between_items');
	    if(!empty($space_between_items_option)) {
		    $space_between_items = $space_between_items_option;
	    }

	    $image_size = 'landscape';
	    $image_size_option = ambient_elated_options()->getOptionValue('portfolio_archive_image_size');
...
if(!function_exists('ambient_elated_get_portfolio_tag_list')) {
	function ambient_elated_get_portfolio_tag_list($tag = '') {
...
if(!function_exists('ambient_elated_single_portfolio')) {
    function ambient_elated_single_portfolio() {
        $portfolio_template = ambient_elated_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'eltdf-portfolio-gallery-' . ambient_elated_options()->getOptionValue('portfolio_single_numb_columns');
        }

        ambient_elated_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('ambient_elated_get_portfolio_single_type')) {
    function ambient_elated_get_portfolio_single_type() {
        return ambient_elated_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('ambient_elated_get_portfolio_single_media')) {
    function ambient_elated_get_portfolio_single_media() {
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'ambient_elated_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = ambient_elated_portfolio_get_video_url($video);
...
if(!function_exists('ambient_elated_portfolio_get_video_url')) {
    function ambient_elated_portfolio_get_video_url($video) {
...
if(!function_exists('ambient_elated_portfolio_get_media_html')) {
    function ambient_elated_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (ambient_elated_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = ambient_elated_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = ambient_elated_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        ambient_elated_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('ambient_elated_compare_portfolio_videos')) {
    /**
     ...
     */
    function ambient_elated_compare_portfolio_videos($a, $b) {
...
if(!function_exists('ambient_elated_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function ambient_elated_compare_portfolio_options($a, $b) {
...
if(!function_exists('ambient_elated_portfolio_get_info_part')) {
    function ambient_elated_portfolio_get_info_part($part) {
        $portfolio_template = ambient_elated_get_portfolio_single_type();

        ambient_elated_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
    }
}