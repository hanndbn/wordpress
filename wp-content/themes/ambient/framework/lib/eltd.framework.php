<?php
/*
   Class: AmbientElatedClassFramework
...
class AmbientElatedClassFramework {
    ...
    private function __construct() {
        $this->eltdOptions = AmbientElatedClassOptions::get_instance();
        $this->eltdMetaBoxes = AmbientElatedClassMetaBoxes::get_instance();
    }
	...
	public function setSkin(AmbientElatedClassSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AmbientElatedClassSkinManager
 *
...
/**
 * Class AmbientElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AmbientElatedClassSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(ambient_elated_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo ambient_elated_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
/*
   Class: AmbientElatedClassOptions
...
class AmbientElatedClassOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $ambient_elated_global_options)) {
            return $ambient_elated_global_options[$key];
...
/*
   Class: AmbientElatedClassAdminPage
   A class that initializes Elated Admin Page
*/
class AmbientElatedClassAdminPage implements iAmbientElatedInterfaceLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new AmbientElatedClassFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iAmbientElatedInterfaceRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: AmbientElatedClassMetaBoxes
...
/*
   Class: AmbientElatedClassMetaBox
   A class that initializes Elated Meta Box
*/
class AmbientElatedClassMetaBox implements iAmbientElatedInterfaceLayoutNode {
	...
	public function setTitle($label) {
		...
		return $ambient_elated_global_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'ambient_elated_init_framework_variable' ) ) {

    function ambient_elated_init_framework_variable() {

        global $ambient_elated_global_Framework;

        $ambient_elated_global_Framework = AmbientElatedClassFramework::get_instance();
        $eltdSkinManager = new AmbientElatedClassSkinManager();
        $ambient_elated_global_Framework->setSkin($eltdSkinManager->getSkin());
    }

    add_action('ambient_elated_before_options_map', 'ambient_elated_init_framework_variable' );
}
?>