...
/*
  Class: AmbientElatedClassIconCollections
...
class AmbientElatedClassIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new AmbientElatedClassIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new AmbientElatedClassIconsFontElegant("Font Elegant", "fe_icon"));
	    $this->addIconCollection('ion_icons', new AmbientElatedClassIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linear_icons', new AmbientElatedClassLinearIcons("Linear Icons", "linear_icon"));
        $this->addIconCollection('linea_icons', new AmbientElatedClassLineaIcons("Linea Icons", "linea_icon"));
    }
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'ambient'),
        ...
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'ambient'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack', 'ambient'),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome' => esc_html__('Font Awesome', 'ambient'),
                'font_elegant' => esc_html__('Font Elegant', 'ambient'),
                'ion_icons'    => esc_html__('Ion Icons', 'ambient'),
                'linea_icons'  => esc_html__('Linea Icons', 'ambient')
            )
        );
    ...
    public function getIconCollectionIcons(iAmbientElatedInterfaceIconCollection $collection) {
...
if ( ! function_exists( 'ambient_elated_activate_theme_icons' ) ) {

    function ambient_elated_activate_theme_icons() {
        global $ambient_elated_global_IconCollections;
        $ambient_elated_global_IconCollections = AmbientElatedClassIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'ambient_elated_activate_theme_icons');
}